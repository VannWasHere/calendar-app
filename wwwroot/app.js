  ...
  scheduler: {
    init(root, dotnet) {
      ...
      root.addEventListener('pointerdown', event => {
        ...
        if (card) { state.id = +card.dataset.booking; state.original = +card.dataset.start; state.duration = +card.dataset.duration; card.setPointerCapture(event.pointerId); card.classList.add('dragging'); }
      ...
      root.addEventListener('pointerup', async () => {
        ...
        if (current.card) { current.card.classList.remove('dragging'); current.card.style.transform = ''; current.card.style.height = ''; }
        let start, end;
